...
static void handleJmpArg(program_t * prog, label_list_t * labels)
{
    ...
    if (strchr(label_str, ':') != NULL){
        ...
        else if (label->ip == NULL){
            logPrint(LOG_DEBUG, "\tfound label %s in list but it is NULL (adding to fixup) (top = %zu)\n", label_str, labels->top);

            labels->fixups[labels->fixup_top].ip = prog->ip;
            labels->fixups[labels->fixup_top].label_id = label - labels->labels;
